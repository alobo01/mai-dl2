...
def get_gtsrb_transforms(img_size): 
    # GTSRB-specific transforms
    train_transform = transforms.Compose([
        transforms.Resize((img_size, img_size)),
        transforms.RandomRotation(15),
        transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),
        transforms.RandomHorizontalFlip(p=0.1),  # Traffic signs shouldn't be flipped too often
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ])
    test_transform = transforms.Compose([
...
def get_transforms(dataset_name: str, img_size: int):
    ...
    elif 'gtsrb' in dataset_name.lower():
        return get_gtsrb_transforms(img_size)
    else:
